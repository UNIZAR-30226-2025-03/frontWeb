         ...
         <div class="modal">
           ...
           <form @submit.prevent="submitNewSong" class="add-song-form">
             ...
             <div class="modal-actions">
               ...
               <button class="cancel-btn" type="button" @click="closeAddSongModal">
                 Cancelar
               </button>
             </div>
           </form>
   ...
   methods: {
     ...
     async submitNewSong() {
       try {
         ...
         const res = await fetch('https://echobeatapi.duckdns.org/admin/songs', {
           method: 'POST',
           headers: { Authorization: `Bearer ${token}` },
           body: form
         })
         if (!res.ok) throw new Error('Error añadiendo canción')
         // Opcional: refrescar listas o notificar éxito
     ...
     async fetchGenres() {
       try {
         const res = await fetch('https://echobeatapi.duckdns.org/genero?userEmail=admin')
         if (!res.ok) throw new Error()
