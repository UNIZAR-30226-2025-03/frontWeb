<template>
   <audio id="app-player" ref="player"></audio> <!-- Asegúrate de que el audio tiene el id y ref correcto -->
 </template>
 ...
 // Variable para almacenar el intervalo de progreso
 ...
 onMounted(() => {
   ...
   // Iniciar el intervalo de progreso
   if (player.value) {
     progressInterval = setInterval(() => {
       // Enviar solo si el audio no está en pausa
       if (!player.value.paused) {
         ...
         console.log(`Progreso enviado: ${currentTime} segundos`)
       }
     }, 2000)
   }
 })
 ...
 onBeforeUnmount(() => {
   ...
   // Detener el streaming si está activo al desmontar el componente
   stopCurrentStream()
 })
 ...
 async function startStreamSong(songId, songName, email) {
   ...
   socket.emit('startStream', { songId: songId, email }) 
 }
